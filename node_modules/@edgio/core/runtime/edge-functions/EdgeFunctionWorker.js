...
// src/constants.ts
var EDGIO_EDGE_FUNCTION_ENV_VARIABLES = {
...
var EdgeFunctionMetrics = class {
  constructor() {
    ...
    this.WASM_METRIC_QUICKJS_FIRST_INDEX = 1e3;
    this.WASM_METRIC_QUICKJS_LAST_INDEX = 1017;
  }
  microseconds() {
    if (performance) {
  ...
  isValidIndex(index) {
    if (index >= this.USER_CUSTOM_METRIC_FIRST_INDEX && index <= this.USER_CUSTOM_METRIC_LAST_INDEX)
      return true;
    if (index >= this.WASM_METRIC_QUICKJS_FIRST_INDEX && index <= this.WASM_METRIC_QUICKJS_LAST_INDEX)
      return true;
    return false;
  }
  isActiveIndex(index) {
...
var EdgeFunctionHeaders = class {
  ...
  allowed_keys() {
    return Object.keys(this._headers).filter(
      (key) => EdgeFunctionHeaders.is_allowed_read_header_name(key)
    );
  }
  remove(name) {
    if (!this.check(name)) {
...
var EdgeFunctionRequest = class {
  ...
  static validate_uri(uri) {
    const MAX_HEADER_BYTES2 = 64 * 1024;
    if (uri.length > MAX_HEADER_BYTES2) {
      return new WasmError(
        8 /* limit_exceeded */,
        `URI length is too long (length ${uri.length} > ${MAX_HEADER_BYTES2})`
      );
    }
...
// src/runtime/edge-functions/EdgeFunctionFetchError.ts
...
var EdgeFunctionFetchError_default = FetchResponseError;
...
var ENV_VAR_NAMESPACE_EDGIO_PROPENVVAR = "edgio-property-environment-variables";
var ENV_VAR_NAMESPACE_EDGIO_FEATURE_FLAGS = "edgio-feature-flags";
var ENV_VAR_NAMESPACE_EDGIO_USRVAR = "edgio-usrvar";
var ENV_VAR_NAMESPACE_EDGIO_GEO = "edgio-geo";
var ENV_VAR_NAMESPACE_EDGIO_WURFL = "edgio-wurfl";
var ENV_VAR_NAMESPACE_EDGIO_VIRT = "edgio-virt";
var ENV_VAR_NAMESPACE_EDGIO_HTTP = "edgio-http";
var SAILFISH_TO_CLI_FEATURE_FLAGS = ["metrics-text-encoding"];
var overflow_message = "<truncated>";
var MAX_OUT_BYTES = 2048;
var MAX_PANIC_BYTES = 512;
var MAX_HEADER_BYTES = 128 * 1024;
var HTTP_HEADER_DELIMITER_BYTES = 4;
var MAX_CREATED_REQUESTS = 10;
var MAX_CREATED_RESPONSES = 10;
var MAX_ISSUED_REQUESTS = 5;
function isHttpVarNameSpace(ns) {
  return ns === ENV_VAR_NAMESPACE_EDGIO_GEO || ns === ENV_VAR_NAMESPACE_EDGIO_VIRT || ns === ENV_VAR_NAMESPACE_EDGIO_WURFL || ns === ENV_VAR_NAMESPACE_EDGIO_HTTP;
}
function getHttpVarPrefix(ns) {
  ...
  if (ns === ENV_VAR_NAMESPACE_EDGIO_WURFL)
    return "wurfl_";
  if (ns === ENV_VAR_NAMESPACE_EDGIO_HTTP)
    return "";
  return "";
}
...
var EdgeFunction = class extends WasmProgram {
  ...
  async run(wasmInitialMemory) {
    ...
    if (wasmInitialMemory === void 0) {
      this.instance.exports.runtime_init();
      const mem = import_buffer4.Buffer.from(this.getGuestMemory().slice(0));
    ...
    const status = this.instance.exports.run(
      this.DOWNSTREAM_REQUEST_ID,
      this.DOWNSTREAM_RESPONSE_ID
    );
  ...
  getQuickJsImportObject() {
    ...
    requests.set(
      this.DOWNSTREAM_REQUEST_ID,
      EdgeFunctionRequest.createFromEdgeFunctionParams(this.params)
    );
    ...
    const startTimeNs = BigInt(Date.now() * 1e6);
    ...
    return {
      env: {
        ...
        hostcall_dict_check: (ns_offset, ns_len, name_offset, name_len) => {
          ...
          if (ns === ENV_VAR_NAMESPACE_EDGIO_FEATURE_FLAGS) {
            return SAILFISH_TO_CLI_FEATURE_FLAGS.indexOf(name) >= 0 ? 0 /* ok */ : 2 /* not_found */;
          }
          if (ns === ENV_VAR_NAMESPACE_EDGIO_CONFIG) {
            return this.params.sysVars.has(name) && name !== EDGIO_EDGE_FUNCTION_ENV_VARIABLES.quickjsBytecodeBase64 ? 0 /* ok */ : 2 /* not_found */;
          }
        ...
        hostcall_dict_get_base64_decoded: (ns_offset, ns_len, name_offset, name_len, value_offset, value_len_offset) => {
          ...
          if (ns === ENV_VAR_NAMESPACE_EDGIO_CONFIG) {
            ...
            if (name instanceof WasmError)
              return handleWasmError(name);
            if (this.params.sysVars.has(name) && name !== EDGIO_EDGE_FUNCTION_ENV_VARIABLES.quickjsBytecodeBase64) {
              ...
              try {
                let decodedValue = decodeBase64ToBuffer(sysVarValue);
        ...
        hostcall_dict_get_key_by_idx: (ns_offset, ns_len, idx, key_offset, key_len_offset) => {
          ...
          if (ns === ENV_VAR_NAMESPACE_EDGIO_USRVAR) {
            ...
            if (idx < usrVarsCount) {
              let writeResult = this.setStringInGuestMemory(
                key_offset,
                key_len_offset,
                Array.from(this.params.usrVars.keys())[idx]
              );
              return writeResult;
            }
            return 2 /* not_found */;
          ...
          if (ns === ENV_VAR_NAMESPACE_EDGIO_FEATURE_FLAGS) {
            ...
            if (idx < featureLength) {
              let writeResult = this.setStringInGuestMemory(
                key_offset,
                key_len_offset,
                SAILFISH_TO_CLI_FEATURE_FLAGS[idx]
              );
              return writeResult;
            }
            return 2 /* not_found */;
          ...
          if (ns === ENV_VAR_NAMESPACE_EDGIO_CONFIG) {
            for (let [key] of this.params.sysVars.entries()) {
              if (key !== EDGIO_EDGE_FUNCTION_ENV_VARIABLES.quickjsBytecodeBase64) {
                if (idx-- === 0) {
          ...
          if (ns === ENV_VAR_NAMESPACE_EDGIO_PROPENVVAR) {
            ...
            if (idx < envVarsCount) {
              let writeResult = this.setStringInGuestMemory(
                key_offset,
                key_len_offset,
                Array.from(this.params.envVars.keys())[idx]
              );
              return writeResult;
            }
            return 2 /* not_found */;
        ...
        hostcall_unique_set_get_key_by_idx: (ns_offset, ns_len, idx, key_offset, key_len_offset) => {
          ...
          if (idx < origins.length) {
            let writeResult = this.setStringInGuestMemory(
              key_offset,
              key_len_offset,
              origins[idx].name || `Undefined origin name at index ${idx}`
            );
            return writeResult;
          }
          return 2 /* not_found */;
        ...
        hostcall_req_create: (method_offset, method_len, url_offset, url_len, req_id_offset) => {
          ...
          if (method instanceof WasmError)
            return handleWasmError(method);
          let url = this.getStringFromGuestMemory(memory, url_offset, url_len);
          if (url instanceof WasmError)
          ...
          if (url.indexOf("://") < 0 && url.indexOf("//") !== 0) {
            const initialRequest = requests.get(this.DOWNSTREAM_REQUEST_ID);
            if (initialRequest) {
              const path = (url.indexOf("/") === 0 ? "" : "/") + url;
              const parsedInitialRequest = (0, import_whatwg_url.parseURL)(initialRequest.url);
              url = `${parsedInitialRequest?.scheme}://${parsedInitialRequest?.host}${path}`;
            }
          }
        ...
        hostcall_resp_streaming_send_to_client: (id) => {
          this.funDebug(`hostcall_resp_streaming_send_to_client(${id})`);
          const response = getResponse(id);
          if (response instanceof WasmError) {
            return handleWasmError(response);
          }
        ...
        hostcall_req_fetch_async: (id) => {
          ...
          this.funDebug(
            `hostcall_req_fetch_async(${id}, url: ${request.url}, method: ${request.method}, headers: ${JSON.stringify(request.getHeaders())}, origin: ${JSON.stringify(request.fetchOrigin)})`
          );
        ...
        hostcall_event_await_next: (event_type_offset) => {
          this.funDebug("hostcall_event_await_next");
          const event_type = this.parent.waitNextIoReadySync();
          const memory = this.getGuestMemory();
          memory.setUint32(event_type_offset, event_type);
          return 0 /* ok */;
        },
        hostcall_event_get_fetch_id: (req_id, resp_id, error_code) => {
          this.funDebug("hostcall_event_get_fetch_id");
          const { requestId, ...response } = this.parent.getNextFetch();
          const memory = this.getGuestMemory();
          memory.setUint32(req_id, requestId);
          if (response.response) {
            const responseId = nextResponseId++;
            this.funDebug(`hostcall_event_get_fetch_id(${requestId}) ${responseId}`);
            memory.setUint32(resp_id, responseId);
            memory.setUint32(error_code, 0);
            ...
            if (!response.error_id && response.error && response.error.code) {
              response.error_id = EdgeFunctionFetchError_default[response.error.code];
            }
            this.funDebug(
              `hostcall_event_get_fetch_id(${requestId}) error: ${JSON.stringify(response)}`
            );
            memory.setUint32(error_code, response.error_id);
            memory.setUint32(resp_id, 0);
          }
          return 0 /* ok */;
        },
        hostcall_event_enable_response_events: (resp_id) => {
          this.funDebug(`hostcall_event_enable_response_events(resp_id: ${resp_id})`);
          const response = getResponse(resp_id);
          if (response instanceof WasmError) {
            return handleWasmError(response);
          }
          this.parent.enableResponseBodyChunkEvents(resp_id);
          if (response.hasChunksToRead()) {
            this.funDebug(
              `hostcall_event_enable_response_events(resp_id: ${resp_id}) calls queueResponseBodyChunkEvent()`
            );
            this.parent.queueResponseBodyChunkEvent(resp_id);
          }
          return 0 /* ok */;
        },
        hostcall_event_response_chunk_get_response_id: (resp_id) => {
          this.funDebug(`hostcall_event_response_chunk_get_response_id()`);
          const responseId = this.parent.getNextBodyChunkResponseId();
          this.funDebug(`hostcall_event_response_chunk_get_response_id() returns ${responseId}`);
          const memory = this.getGuestMemory();
          memory.setUint32(resp_id, responseId);
          return 0 /* ok */;
        },
        hostcall_resp_get_next_body_chunk: (resp_id, data_offset, len_offset) => {
          this.funDebug(`hostcall_resp_get_next_body_chunk(resp_id: ${resp_id})`);
          const response = getResponse(resp_id);
          if (response instanceof WasmError) {
            return handleWasmError(response);
          }
          ...
          this.funDebug(
            `hostcall_resp_streaming_get_body_chunk_async(${resp_id}, ${len}) ${response.body?.byteLength}`
          );
          const chunk = response.readStreamingBodyChunk(len);
          if (chunk === void 0) {
            return 2 /* not_found */;
          }
        ...
        hostcall_req_set_uri: (id, data_offset, len) => {
          ...
          request.url = uri;
          request.path = new URL(uri).pathname;
        ...
        hostcall_req_set_fetch_ckqs_include: (id, params_offset, params_length) => {
          ...
          if (params instanceof WasmError)
        ...
        hostcall_req_set_fetch_max_age: (id, maxage_offset, maxage_length) => {
          ...
          if (maxage instanceof WasmError)
        ...
        hostcall_req_set_fetch_cache_tags: (id, cache_tags_offset, cache_tags_length) => {
          ...
          const memory = this.getGuestMemory();
          const cacheTags = this.getStringFromGuestMemory(
            memory,
            cache_tags_offset,
            cache_tags_length
          );
          if (cacheTags instanceof WasmError)
        ...
        hostcall_req_set_fetch_origin: (id, origin_offset, len) => {
          ...
          if (fetchOrigin instanceof WasmError)
        ...
        hostcall_req_append_body: (id, ptr, len) => {
          ...
          if (request.rawBody === void 0) {
            ...
            request.rawBody = import_buffer4.Buffer.concat([request.rawBody, import_buffer4.Buffer.from(body)]);
          }
          return 0 /* ok */;
        },
        hostcall_req_get_body: (id, data_offset, len_offset) => {
          ...
          return this.copyDataToGuestMemory(
            request.rawBody || import_buffer4.Buffer.alloc(0),
            data_offset,
            len_offset
          );
        },
        hostcall_req_get_body_chunk: (id, chunk_offset, data_offset, len_offset) => {
          ...
          const len = this.getGuestMemory().getUint32(len_offset);
          this.funDebug(
            `hostcall_req_get_body_chunk(${id}, ${chunk_offset}, ${len}) ${request.rawBody?.byteLength}`
          );
        ...
        hostcall_resp_append_body: (id, ptr, len) => {
          ...
          if (response.responseBodyFinalized) {
            return handleWasmError({
              status: 6 /* invalid_operation */,
              message: "Cannot append to a response that has been finalized."
            });
          }
          const memory = this.getGuestMemory();
          const body = memory.slice(ptr, ptr + len);
          if (response.body === void 0) {
            ...
            response.body = import_buffer4.Buffer.concat([response.body, import_buffer4.Buffer.from(body)]);
          }
          return 0 /* ok */;
        },
        hostcall_resp_streaming_finalize_send_body: (id, ptr, len) => {
          const response = getResponse(id);
          if (response instanceof WasmError) {
            return handleWasmError(response);
          }
          response.responseBodyFinalized = true;
          return 0 /* ok */;
        },
        ...
        hostcall_req_get_header_values: (id, key_offset, key_length, sizes_offset, sizes_length_offset, values_offset, values_length_offset) => {
          ...
          let rc = this.copyDataToGuestMemory(
            this.stringLengthsToBuffer(headerValues),
            sizes_offset,
            sizes_length_offset
          );
          if (rc !== 0 /* ok */)
            return rc;
          return this.copyDataToGuestMemory(
            import_buffer4.Buffer.from(headerValues.join("")),
            values_offset,
        ...
        hostcall_req_set_header: (id, key_offset, key_length, val_offset, val_length) => {
          ...
          if (request.headers.get_total_bytes() + key_length + val_length + HTTP_HEADER_DELIMITER_BYTES > MAX_HEADER_BYTES) {
            return handleWasmError({
              status: 8 /* limit_exceeded */,
        ...
        hostcall_resp_set_header: (id, key_offset, key_length, val_offset, val_length) => {
          ...
          if (response.headers.get_total_bytes() + key_length + val_length + HTTP_HEADER_DELIMITER_BYTES > MAX_HEADER_BYTES) {
            return handleWasmError({
              status: 8 /* limit_exceeded */,
      ...
      wasi_snapshot_preview1: {
          ...
          for (let i = 0; i < iovs_length; i++) {
            ...
            switch (fd) {
              case STDOUT_FD: {
                ...
                if (typeof window === void 0 && process?.env?.LOG_LEVEL === "trace") {
        ...
        fd_read: (...args) => {
          this.funDebug("fd_read not supported");
        ...
        sched_yield: (...args) => {
          this.funDebug("sched_yield not supported");
...
// src/runtime/edge-functions/consts.ts
var EVENT_ERROR = -1;
var EVENT_IO_FETCH = 1;
var EVENT_IO_RESPONSE_CHUNK = 2;
...
var SEMAPHORE_INDEX_WORKER_WAIT = 0;
var SEMAPHORE_VALUE_SHOULD_WAIT = 0;
...
var EdgeFunctionParentInterface = class {
    ...
    this.port.onmessage = async (event) => {
      ...
      if (command === "run") {
        ...
        try {
          ...
          if (response === EdgeFunctionError_default.ErrorRunning) {
  ...
  waitNextIoReadySync() {
    ...
    Atomics.store(semaphore, SEMAPHORE_INDEX_WORKER_WAIT, SEMAPHORE_VALUE_SHOULD_WAIT);
    this.port.postMessage(["waiting"]);
    this.writeToDebugStream(import_buffer5.Buffer.from(`waitNextIoReadySync(): Block until the next event...`));
    Atomics.wait(semaphore, SEMAPHORE_INDEX_WORKER_WAIT, SEMAPHORE_VALUE_SHOULD_WAIT);
    this.writeToDebugStream(import_buffer5.Buffer.from(`waitNextIoReadySync(): received event.`));
    const length = dataLength[0];
    const message = Serializer.deserialize(import_buffer5.Buffer.from(exchangeBuffer, 0, length));
    switch (message.event) {
      case EVENT_ERROR:
        this.writeToDebugStream(import_buffer5.Buffer.from(`waitNextIoReadySync(): EVENT_ERROR: ${message.error}`));
        throw new Error(`Error in the worker: ${message.error}`);
      case EVENT_IO_FETCH:
        if (message.response?.body) {
          message.response.body = import_buffer5.Buffer.from(message.response?.body);
        }
    ...
    this.writeToDebugStream(
      import_buffer5.Buffer.from(`waitNextIoReadySync(): returns ${eventToString(message.event)}`)
    );
  ...
  getNextBodyChunkResponseId() {
    if (this.response_chunks_fifo_queue.length === 0) {
      throw new Error(
        `hostcall_event_response_chunk_get_response_id: There are no pending response body chunks ready.`
      );
    }
  ...
  get path() {
    return this.workerData?.params.path ?? "Unknown";
  }
};
