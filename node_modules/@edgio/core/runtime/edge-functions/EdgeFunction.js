...
const ENV_VAR_NAMESPACE_EDGIO_HTTP = 'edgio-http';
...
function isHttpVarNameSpace(ns) {
    return (ns === ENV_VAR_NAMESPACE_EDGIO_GEO ||
        ...
        ns === ENV_VAR_NAMESPACE_EDGIO_HTTP);
}
        ...
        return {
            env: {
                ...
                hostcall_req_create: (method_offset, method_len, url_offset, url_len, req_id_offset) => {
                    ...
                    // In order to maintain compatibility with Sailfish, we allow for the case
                    // where the user calls fetch('/some/path') without protocol and host. In this
                    // case we use the host and protocol from the initial request.
                    if (url.indexOf('://') < 0 && url.indexOf('//') !== 0) {
                        const initialRequest = requests.get(this.DOWNSTREAM_REQUEST_ID);
                        if (initialRequest) {
                            // Ensure the path starts with a slash
                            const path = (url.indexOf('/') === 0 ? '' : '/') + url;
                            const parsedInitialRequest = (0, whatwg_url_1.parseURL)(initialRequest.url);
                            url = `${parsedInitialRequest === null || parsedInitialRequest === void 0 ? void 0 : parsedInitialRequest.scheme}://${parsedInitialRequest === null || parsedInitialRequest === void 0 ? void 0 : parsedInitialRequest.host}${path}`;
                        }
                    }
                ...
                hostcall_req_fetch_async: (id) => {
                    ...
                    // This is very useful when debugging locally.
                    this.funDebug(`hostcall_req_fetch_async(${id}, url: ${request.url}, ` +
                        `method: ${request.method}, headers: ${JSON.stringify(request.getHeaders())}, ` +
                        `origin: ${JSON.stringify(request.fetchOrigin)})`);
                ...
                // Received fetch complete event, so get fetch id
                hostcall_event_get_fetch_id: (req_id, resp_id, error_code) => {
                    ...
                    if (response.response) {
                        ...
                        this.funDebug(`hostcall_event_get_fetch_id(${requestId}) error: ${JSON.stringify(response)}`);
                        memory.setUint32(error_code, response.error_id);
                        memory.setUint32(resp_id, 0);
                    }
                    return WasmProgram_1.wasm_status.ok;
                },
                // Enable response body streaming events for given response
                hostcall_event_enable_response_events: (resp_id) => {
                    ...
                    if (response.hasChunksToRead()) {
                        this.funDebug(`hostcall_event_enable_response_events(resp_id: ${resp_id}) calls queueResponseBodyChunkEvent()`);
                        this.parent.queueResponseBodyChunkEvent(resp_id);
                    }
                    return WasmProgram_1.wasm_status.ok;
                },
                // We received a body streaming event, the gets response id for that event
                ...
                // We eceived body streaming event, got the resp_id, not get next streaming body chunk for that resp_id
                hostcall_resp_get_next_body_chunk: (resp_id, data_offset, len_offset) => {
                    ...
                    this.funDebug(`hostcall_resp_streaming_get_body_chunk_async(${resp_id}, ${len}) ${(_a = response.body) === null || _a === void 0 ? void 0 : _a.byteLength}`);
