...
class DeploymentBuilder {
    ...
    async addProdEntrypoint(connector) {
        ...
        // Try to locate the prod entry file
        const prod = (0, paths_1.getEntryPoint)('prod', this.appDir, connector);
        if (prod !== undefined)
            this.addInternalJSAsset(prod, (0, paths_1.pathForBackend)(constants_1.BACKENDS.js));
        else {
            // If we didn't found connector's prod.js file, we fallback
            // to core's prod.js file, as we allow connectors to NOT
            // specify a prod.js file. We cannot do this in a runtime,
            // as AWS runtime doesn't contain "node_modules", so we cannot
            // fallback to @edgio/core, so we include it on deployment build
            const coreProd = (0, paths_1.getEntryPoint)('prod', this.appDir);
            if (coreProd !== undefined)
                this.addInternalJSAsset(coreProd, (0, paths_1.pathForBackend)(constants_1.BACKENDS.js));
            else
                throw new Error('No production entry point found');
        }
        return this;
    }
    /**
    ...
    addDefaultAppResources(defaultAppPath, replacer) {
        // Checks if given file exists in the app directory
        // by comparing the file name without the extension.
        // For example: resourceExists('next.config.js') will return true
        // if any of 'next.config.js', 'next.config.mjs' or 'next.config.cjs' exists in the app directory.
        const resourceExists = (file) => {
            const appFiles = (0, fs_extra_1.readdirSync)(this.appDir);
            const fileWithoutExtension = (0, path_1.basename)(file).replace(/\.[^/.]+$/, '');
            for (const appFile of appFiles) {
                const appFileWithoutExtension = (0, path_1.basename)(appFile).replace(/\.[^/.]+$/, '');
                if (appFileWithoutExtension === fileWithoutExtension) {
                    return true;
                }
            }
        };
        // Adds resources from the default app directory to the user's app directory
        const addResources = (fromPath) => {
            for (let file of globby_1.default.sync('**/*', {
                cwd: fromPath,
            })) {
                if (!resourceExists((0, path_1.join)(this.appDir, file))) {
        ...
        const hasMjsResources = (0, fs_extra_1.existsSync)((0, path_1.join)(defaultAppPath, 'mjs'));
        const hasTsResources = (0, fs_extra_1.existsSync)((0, path_1.join)(defaultAppPath, 'ts'));
        const isTypeModule = (0, getProjectType_1.default)() === 'module';
        const isTsProject = (0, fs_extra_1.existsSync)((0, path_1.join)(process.cwd(), 'tsconfig.json'));
        // Add resources for all project types
        addResources((0, path_1.join)(defaultAppPath, 'all'));
        // Add resources for specific project types
        if (isTsProject && hasTsResources) {
            addResources((0, path_1.join)(defaultAppPath, 'ts'));
        }
        else if (isTypeModule && hasMjsResources) {
            addResources((0, path_1.join)(defaultAppPath, 'mjs'));
        }
        else {
     ...
     */
    async build(options) {
        const { router, config, srcRouterPath } = await this.buildSources();
        await this.addMetadataFiles(srcRouterPath, router);
    ...
    async createServerlessBundle({ serverless, connector }, router, options) {
        ...
        // Add bundled lambda handler to internal js directory,
        ...
        this.copySync((0, path_1.join)(__dirname, '..', 'lambda', 'handler.dist.js'), (0, path_1.join)(this.jsInternalDir, 'handler.cjs'));
        this.copySync((0, path_1.join)(__dirname, '..', 'lambda', 'handler.dist.js.map'), (0, path_1.join)(this.jsInternalDir, 'handler.cjs.map'));
        // Add bundled lambda server to internal js directory
        this.copySync((0, path_1.join)(__dirname, '..', 'lambda', 'server.dist.js'), (0, path_1.join)(this.jsInternalDir, 'server.cjs'));
        this.copySync((0, path_1.join)(__dirname, '..', 'lambda', 'server.dist.js.map'), (0, path_1.join)(this.jsInternalDir, 'server.cjs.map'));
        // Re-export the internal handler from the root of the lambda directory
        this.writeFileSync((0, path_1.join)(this.lambdaDir, 'handler.js'), `module.exports = require('./internal/handler.cjs')`);
        ...
        if ((0, edgeFunctionUtils_1.containsEdgeFunctionFeature)(router.rules)) {
            ...
            const quickjsRuntimeFilename = 'quickjs-llrt-runtime.wasm';
    ...
    async buildSources() {
        ...
        const srcRouterPath = (0, paths_1.getSrcRouterPath)(config);
    ...
    async addMetadataFiles(srcRouterPath, router) {
        const routerInfo = (0, parseRouter_1.default)(srcRouterPath);
        // routerInfo.json
        this.writeFileSync((0, path_1.join)(this.buildDir, 'routerInfo.json'), JSON.stringify({ location: (0, path_1.relative)(this.appDir, srcRouterPath), info: routerInfo }, null, 2));
        // EDGIO_VERSION
        ...
        const edgeFunctionSources = {
            sources: edgeFunctionsNodeBuilder.sources,
            // Leave a hint on the edge function init script
            initScript: router.routerOptions.edge_function_init_script,
        };
        this.writeFileSync((0, path_1.join)((0, paths_1.getJsInternalPath)(), paths_1.EDGE_FUNCTION_SOURCES_FILENAME), JSON.stringify(edgeFunctionSources, null, 2));
