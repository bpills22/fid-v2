...
class PrefetchPlugin {
    ...
    async cacheWillUpdate({ response, request, event }) {
        ...
        // Ensure that the response is not a browser challenge.
        if (await this.isBrowserChallenge(request, response)) {
            return null;
        }
        // Response with no size should not be cached
    ...
    /**
     * Check if the response is a browser challenge.
     * @private
     * @param {Request} request
     * @param {Response} response
     * @returns {Promise<boolean>}
     */
    async isBrowserChallenge(request, response) {
        var _a;
        // Only check for browser challenges on text/html responses.
        if (!((_a = response.headers.get('content-type')) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes('text/html'))) {
            return false;
        }
        // Known body content as part of the default browser challenge
        const bodyMatches = [/onloadChallenge\(\)/, /__eccha/];
        // TODO: Check headers for known browser challenge headers.
        // Known headers that indicate a browser challenge.
        // const headerMatches: RegExp[] = []
        // const headers = new Headers(response.headers)
        // // Check headers for known browser challenge headers.
        // for (const [header, value] of headers.entries()) {
        //   if (headerMatches.some(re => re.test(header)) || headerMatches.some(re => re.test(value))) {
        //     return true
        //   }
        // }
        // Check body for known browser challenge content.
        const clonedResponse = response.clone();
        const text = await clonedResponse.text();
        return bodyMatches.every(re => re.test(text));
    }
}
