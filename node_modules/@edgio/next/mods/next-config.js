...
function transformNextConfig(fileInfo, api) {
    // Do not transform the next config file multiple times
    if (fileInfo.source.includes('withEdgio'))
        return fileInfo.source;
    const filename = (0, path_1.basename)(fileInfo.path);
    ...
    if (!defaultExportValue)
        return fileInfo.source;
    const isFunction = ['FunctionDeclaration', 'ArrowFunctionExpression'].includes(defaultExportValue.type);
    const edgioNextConfigRoot = j((0, fs_extra_1.readFileSync)((0, path_1.join)(__dirname, '..', 'default-app', fileExtension, `next.config.${fileExtension}`)).toString());
    ...
    if (!edgDefaultExportValue)
        return fileInfo.source;
    // Move existing user's default export under const _preEdgioExport = ...
    const existingExportVar = j.identifier(OLD_EXPORT_VAR_NAME);
    defaultExport.replaceWith(j.variableDeclaration('const', [j.variableDeclarator(existingExportVar, defaultExportValue)]));
    ...
    const existingResultExpr = isFunction
        ? j.callExpression(existingExportVar, edgDefaultExportValue.params)
        : existingExportVar;
    ...
    const withEdgioExpr = edgioNextConfigRoot.find(j.CallExpression, (expr) => isIdentifier(expr.callee) && expr.callee.name === 'withEdgio');
    ...
    const newExportDefault = fileExtension === 'js'
        ? j.expressionStatement(j.assignmentExpression('=', j.memberExpression(j.identifier('module'), j.identifier('exports')), edgDefaultExportValue))
    ...
    edgioNextConfigRoot
        ...
        const isEdgioExport = node.type === edgDefaultExport.get().value.type &&
            node.start === edgDefaultExport.get().value.start;
        // Common JS default export
        const isModuleExports = node.type === 'ExpressionStatement' &&
        ...
        if (isModuleExports || isEdgioExport)
            return;
    ...
    const sourceCode = root
        ...
        .replace(`..._preEdgioExport(_phase: string, _config: any)`, '..._preEdgioExport(_phase, _config)');
...
exports.default = transformNextConfig;
